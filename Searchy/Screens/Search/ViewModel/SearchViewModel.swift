...
extension SearchViewModel {
    func fetchResults(for searchText: String?) {
        ...
        networkManager.getSearchResults(for: searchText) { [weak self] searchModel in
            ...
            if let totalResults = searchModel?.totalResults,
...
extension SearchViewModel {
    ...
    private func getSearchResultCellViewModels(
        for searchResultModels: [SearchResultModel]
    ) -> [
        BaseCellViewModelProtocol
    ] {
        var cellViewModels: [BaseCellViewModelProtocol] = []
...
extension SearchViewModel {
    ...
    func didSelectItemAt(_ indexPath: IndexPath) {
        let cellViewModel = cellViewModels[indexPath.section][indexPath.row]

        guard
            let searchResultCellViewModel = cellViewModel as? SearchResultCellViewModelProtocol,
            let imdbID = searchResultCellViewModel.imdbID
        else { return }
